...
{fetchArray.map((item) => {
  // Check if item.image and item.image.data are defined
  const base64String = item.image && item.image.data
    ? btoa(String.fromCharCode(...new Uint8Array(item.image.data.data)))
    : '';  // Fallback to an empty string if image data is not available
