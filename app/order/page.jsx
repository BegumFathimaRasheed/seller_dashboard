  ...
  return (
    <div className="flex bg-white body">
      ...
      <div className="flex-grow" style={{ backgroundColor: 'rgba(62, 98, 151, 0.103)' }}>
        ...
        <div className="p-5" style={{ backgroundColor: 'rgb(94, 16, 94)' }}></div>
          ...
          {!loading && (
            <div className="container flex sm:flex-row">
              {fetchArray.map((item) => {
                ...
                const base64String = item.image && item.image.data && item.image.data.data
                  ? btoa(String.fromCharCode(...new Uint8Array(item.image.data.data)))
                  ...
                  <Link href={''} key={item._id} className="card bid sm:w-80 sm:h-100 shadow-brown rounded-lg overflow-hidden transition-transform transform hover:scale-105 bg-white"
                    ...
                    <div style={{ display: 'flex', justifyContent: 'center' }}>
                      <div style={{ padding: '0.5rem', height: '5rem', width: '5rem', paddingTop: '1rem' }}>
                        <Image 
                          src={base64String ? `data:image/png;base64,${base64String}` : '/path/to/placeholder.png'} 
                          alt="Product Image" 
                          className="image" 
                          width={0} 
                          height={0} 
                    ...
                    <div style={{ paddingLeft: '1rem', paddingRight: '1rem', paddingBottom: '1rem' }}>
                      ...
                      <h3 style={{ fontSize: '0.8rem', color: 'gray', marginTop: '1rem' }}>CUSTOMER NAME - <span className="font-bold" style={{ fontSize: '0.8rem', color: 'black', marginLeft: '0.5rem' }}>{item.cname}</span></h3>
                      <h3 style={{ fontSize: '0.8rem', color: 'gray', marginTop: '0.5rem' }}>CONTACT NO - <span className="font-bold" style={{ fontSize: '0.8rem', color: 'black', marginLeft: '0.5rem' }}>{item.contact}</span></h3>
                      <h3 style={{ fontSize: '0.8rem', color: 'gray', marginTop: '0.5rem' }}>MRP PRICE - <span className="font-bold" style={{ fontSize: '0.8rem', color: 'black', marginLeft: '0.5rem' }}>LKR. {item.mrp}</span></h3>
                      <h3 style={{ fontSize: '0.8rem', color: 'gray', marginTop: '0.5rem' }}>SALE PRICE - <span className="font-bold" style={{ fontSize: '0.8rem', color: 'black', marginLeft: '0.5rem' }}>LKR. {item.sellp}</span></h3>
                      <h3 style={{ fontSize: '0.8rem', color: 'gray', marginTop: '0.5rem', display: 'flex' }}>DELIVERY DATE - <span className="font-bold flex" style={{ fontSize: '0.8rem', color: 'black', marginLeft: '0.5rem' }}><FaRegCalendarAlt size={16} className="mr-1" />{item.date}</span></h3>
                    </div>
                  </Link>
                )
              })}
